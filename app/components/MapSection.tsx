...
const MapSection = ({ points }: { points: Point[] }) => {
  ...
  const { initializeRTLPlugin, customizeMapLayers } = useMapStore();
  ...
  const center = useMemo(() => {
    const first = points.find(
      (p) =>
        ...
        !isNaN(Number(p.location.long))
    );
    return first
      ? [Number(first.location.long), Number(first.location.lat)]
      : [34.825939, 32.2337946];
  }, [points]);
  ...
  useEffect(() => {
    ...
    points.forEach((point) => {
      ...
      if (!isNaN(lat) && !isNaN(lng)) {
        ...
        // Create the marker icon element using custom MarkerSvg
        const markerEl = document.createElement('div');
        markerEl.innerHTML = ReactDOMServer.renderToString(<MarkerSvg />);
        markerEl.style.display = 'flex';
        markerEl.style.justifyContent = 'center';
        markerEl.style.alignItems = 'center';

        // Style the marker based on disabled state
        if (point.disabled) {
        ...
        // Add click behavior to the label (point)
        if (point.disabled) {
